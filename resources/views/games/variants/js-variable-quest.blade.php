...
@section('content')
@vite('resources/js/games/js-quest.js')
<div x-data="gameLogic()" class="flex flex-col lg:flex-row h-screen lg:h-full w-full bg-[#f3f4f6] relative overflow-y-auto lg:overflow-hidden">
    ...
    <div class="w-full lg:w-[30%] lg:h-full p-4 sm:p-6 border-b-4 lg:border-b-0 lg:border-r-4 border-black flex flex-col bg-white lg:overflow-y-auto">
        ...
        <div class="mb-6">
            <div class="bg-[#1a1a1a] neo-border neo-shadow p-4 rounded-none relative">
                <div class="text-gray-400 font-mono text-sm mb-2">// index.js</div>
                <textarea 
                    x-model="userCode" 
                    @keyup.debounce.500ms="runCode()"
                    class="w-full bg-transparent text-[#F7DF1E] font-mono text-base resize-none outline-none focus:ring-0 border-none p-0 min-h-[160px] lg:min-h-[120px]"
                    spellcheck="false"
                ></textarea>
            </div>
        </div>
        ...
        <div class="flex gap-4 mt-auto">
            ...
            <button class="px-5 bg-white neo-border neo-shadow neo-button-hover font-bold text-lg uppercase h-[56px]" @click="runCode()">
                ▶ Run
            </button>
            <button @click="showHint = !showHint" class="px-5 neo-border neo-shadow neo-button-hover font-bold text-lg uppercase h-[56px] transition-colors" :class="showHint ? 'bg-[#FFE500]' : 'bg-white'">
                <span x-show="!showHint">💡</span>
                <span x-show="showHint">✕</span>
            </button>
        </div>
    ...
    <!-- CENTER PANEL (40%) -->
    <div class="w-full lg:w-[40%] lg:h-full p-4 sm:p-8 flex flex-col items-center justify-center bg-[#F7DF1E] border-b-4 lg:border-b-0 lg:border-r-4 border-black relative overflow-hidden min-h-[350px]">
        
        <!-- Console Preview -->
        <div class="w-full h-full max-w-2xl bg-[#282a36] neo-border neo-shadow flex flex-col" id="preview-container">
            <!-- Terminal Chrome -->
            <div class="bg-[#44475a] border-b-4 border-black p-3 flex gap-2 items-center">
                <div class="text-[#f8f8f2] font-mono text-sm font-bold flex-1">
                    > Terminal
                </div>
            </div>
            
            <div id="console-output" class="flex-grow p-4 overflow-y-auto text-sm pb-8">
                <!-- Messages appended here by JS -->
            </div>
            
            <!-- Hidden Sandbox Iframe -->
            <iframe id="sandbox-frame" class="hidden" sandbox="allow-scripts"></iframe>
        </div>
...
<script>
    function gameLogic() {
        return {
            ...
            runCode() {
                if (window.runJsCode) {
                    window.runJsCode(this.userCode);
                }
            },
                        ...
                        if (this.roomCode) {
                            ...
                            if (data.success) {
                                ...
                                if (isCorrect) {
                                    ...
                                    if (preview) { preview.classList.add('scale-105', 'transition-transform'); setTimeout(() => preview.classList.remove('scale-105'), 300); }
