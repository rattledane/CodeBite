@if(request()->query('room'))
    <script>
        window.roomParticipants = @json(\App\Models\Room::where('code', request()->query('room'))->first()?->participants()->with('user')->get() ?? []);
        window.isRoomHost = {{ \App\Models\Room::where('code', request()->query('room'))->first()?->host_id === auth()->id() ? 'true' : 'false' }};
    </script>
@endif

<script>
    function multiplayerBoard(roomCode) {
        return {
            roomCode: roomCode,
            players: [],
            myRank: 0,
            ...
            init() {
                ...
                if (window.Echo) {
                    window.Echo.channel(`room.${this.roomCode}`)
                        ...
                        .listen('.room.restarted', (e) => {
                            window.location.href = `/rooms/${this.roomCode}/lobby`;
                        });
                }
            },
            ...
            isHost() {
                return window.isRoomHost === true;
            },

            async restartRoom() {
                try {
                    await fetch(`/rooms/${this.roomCode}/restart`, {
                        method: 'POST',
                        headers: {
                    ...
                    console.error('Failed to restart room', e);
                }
            }
        };
    }
